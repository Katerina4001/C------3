...
public class Toys {
    ...
    public Toys(String name, int count, int weight)  throws ToysIsNotCreateException {
        int error = 0;
        toys.add(this);
        this.id = toys.size() - 1;
        error += setName(name);
        ...
        if (error != 0) {throw new ToysIsNotCreateException(error, "Ошибка создания экземпляра класса");}
    }
